...
echo "  -> Bootstrapping environment (traced: OTLP + pretty) ..."
devenv allow
# Trace from the moment devenv lands. Spans (Nix eval, derivation build, the
# bootstrap task, and every process it launches) flow to any OTLP collector on
# :4317 (e.g. a local jaeger / Tempo). devenv also forwards TRACEPARENT /
# TRACESTATE to child processes, so the whole bootstrap joins one trace.
export DEVENV_TRACE_TO="pretty:stderr,otlp-grpc"
export OTEL_EXPORTER_OTLP_ENDPOINT="${OTEL_EXPORTER_OTLP_ENDPOINT:-http://localhost:4317}"
devenv shell -- just bootstrap
